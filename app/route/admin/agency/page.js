...
const AgencyDetails = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const response = await axios.get('http://localhost:8080/api/client/agency', {
          headers: {
  ...
  return (
    <div className="container mx-auto mt-20 py-10">
      ...
      {loading ? (
        <p className="text-center text-blue-600">Loading agencies...</p>
      ) : error ? (
        <p className="text-center text-red-500">{error}</p>
      ) : users.length === 0 ? (
        <p className="text-center text-gray-500">No agency found.</p>
      ) : (
